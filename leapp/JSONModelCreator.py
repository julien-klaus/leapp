...
class JSONModelCreator(object):
    ...
    def generate_model_as_json_with_r(self, install_packages=False, verbose=False):
        vars = self.get_vars()
        for var in vars:
        ...
        if install_packages:
            r_parameter["install"] = "install.packages(c('jsonlite','bnlearn'))"
        else:
            r_parameter["install"] = ""
        bnlearn_code = """
            {install}
        ...
        # if windows we have to change / to //
        if os.name == "nt":
